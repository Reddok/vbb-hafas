...
const route = (l, p, r) => {
	...
	return (d) => {
		const r = parseRoute(d)
		if (!r) return null

		if (d.trfRes && d.trfRes.fareSetL && d.ovwTrfRefL) {
			r.tickets = []

			for (let t of d.ovwTrfRefL) {
				const set = d.trfRes.fareSetL[t.fareSetX]
				if (!set || !Array.isArray(set.fareL)) continue
				const fare = set.fareL[t.fareX]
				if (!fare || !Array.isArray(fare.ticketL)) continue
				const raw = fare.ticketL[t.ticketX]
				if (!raw) continue

				t.tickets.push(parseTicket(raw.name, raw.prc, raw.shpCtx))
			}
		}
